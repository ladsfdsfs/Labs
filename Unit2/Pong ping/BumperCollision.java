...
public class BumperCollision
{
   ...
   public static void collide(Bumper bumper, Ball ball, int change, int hits, Bumper b2)
   {
      ...
      {	   	
         ...
         if(change==JOptionPane.NO_OPTION){
            if(bumper.getX() > 399){ 
            double d = distance(bumper.getX(),bumper.getY(), enemy.getX(), enemy.getY());
            if(
            }
         }
         ball.setFlash(ball.getFlash()+1);
